...
with open("ST258_Mero_CoreAndAGEs_EN_f1Tuned_Xtrain_split{0}.pkl".format(str(args.split)), "rb") as f:
    Xtrain = pickle.load(f)
print(Xtrain.head(), flush=True)
with open("ST258_Mero_CoreAndAGEs_EN_f1Tuned_ytrain_split{0}.pkl".format(str(args.split)), "rb") as f:
    ytrain = pickle.load(f)
print(ytrain.head(), flush=True)

print("Test set:", flush=True)
with open("ST258_Mero_CoreAndAGEs_EN_f1Tuned_Xtest_split{0}.pkl".format(str(args.split)), "rb") as f:
    Xtest = pickle.load(f)
print(Xtrain.head(), flush=True)
with open("ST258_Mero_CoreAndAGEs_EN_f1Tuned_ytest_split{0}.pkl".format(str(args.split)), "rb") as f:
    ytest = pickle.load(f)
print(ytest.head(), flush=True)

# Load in SNV and UG feature lists
print("Feature Lists:")
with open("ST258_Mero_SNV_features.pkl".format(str(args.split)), "rb") as f:
    SNV_features = pickle.load(f)
print(SNV_features[0:10], flush=True)
with open("ST258_Mero_UG_features.pkl".format(str(args.split)), "rb") as f:
...
# Create pipeline to do both one-hot encoding and then EN
pipe_CG_AG = Pipeline(steps=[
    ("transformer", transformer),
    ("en", LogisticRegression(penalty="elasticnet", solver="saga", max_iter=10000,random_state=s))
])
print("Pipeline:", flush=True)
print(pipe_CG_AG, flush=True)

# Create GSCV estimator
param_grid = {'en__C': [10 ** x for x in range(-4,5)],
              'en__l1_ratio': [0.1 * x for x in range(11)],
              'en__class_weight': ['balanced', None]}
...
with open('ST258_Mero_CoreAndAGEs_EN_f1Tuned_split{0}_GridSearch.pkl'.format(str(args.split)), 'wb') as f:
...
Scores.to_csv("ST258_Mero_CoreAndAGEs_EN_f1Tuned_split{0}_CVResults.csv".format(str(args.split)), index=False)
