...
with open("ST258_Mero_Curated_LR_f1Tuned_Xtrain_split{0}.pkl".format(str(args.split)), "rb") as f:
    Xtrain = pickle.load(f)
print(Xtrain.head(), flush=True)
with open("ST258_Mero_Curated_LR_f1Tuned_ytrain_split{0}.pkl".format(str(args.split)), "rb") as f:
    ytrain = pickle.load(f)
print(ytrain.head(), flush=True)

print("Test set:", flush=True)
with open("ST258_Mero_Curated_LR_f1Tuned_Xtest_split{0}.pkl".format(str(args.split)), "rb") as f:
    Xtest = pickle.load(f)
print(Xtrain.head(), flush=True)
with open("ST258_Mero_Curated_LR_f1Tuned_ytest_split{0}.pkl".format(str(args.split)), "rb") as f:
    ytest = pickle.load(f)
...
LR_model = LogisticRegression(penalty="l2", solver="lbfgs", max_iter=10000,random_state=s)

# Create GSCV estimator
param_grid = {'C': [10 ** x for x in range(-4,5)],
              'class_weight': ['balanced', None]}
GSCV_Curated = GridSearchCV(LR_model, param_grid, scoring="f1", cv=stratkf, iid=False, n_jobs=-1)
...
with open('ST258_Mero_Curated_LR_f1Tuned_split{0}_GridSearch.pkl'.format(str(args.split)), 'wb') as f:
...
Scores.to_csv("ST258_Mero_Curated_LR_f1Tuned_split{0}_CVResults.csv".format(str(args.split)), index=False)
