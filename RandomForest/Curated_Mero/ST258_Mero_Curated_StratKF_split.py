...
for train_index, test_index in stratkf.split(X, y):
    ...
    with open('ST258_Mero_Curated_RF_f1Tuned_Xtrain_split{0}.pkl'.format(str(split)), 'wb') as f:
        pickle.dump(Xtrain, f)
    with open('ST258_Mero_Curated_RF_f1Tuned_ytrain_split{0}.pkl'.format(str(split)), 'wb') as f:
        pickle.dump(ytrain, f)
    with open('ST258_Mero_Curated_RF_f1Tuned_Xtest_split{0}.pkl'.format(str(split)), 'wb') as f:
        pickle.dump(Xtest, f)
    with open('ST258_Mero_Curated_RF_f1Tuned_ytest_split{0}.pkl'.format(str(split)), 'wb') as f:
        pickle.dump(ytest, f)
    split = split + 1
