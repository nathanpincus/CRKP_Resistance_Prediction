...
with open("ST258_Mero_CoreAndAGEs_SVC_f1Tuned_Xtrain_split{0}.pkl".format(str(args.split)), "rb") as f:
    Xtrain = pickle.load(f)
print(Xtrain.head(), flush=True)
with open("ST258_Mero_CoreAndAGEs_SVC_f1Tuned_ytrain_split{0}.pkl".format(str(args.split)), "rb") as f:
    ytrain = pickle.load(f)
print(ytrain.head(), flush=True)

print("Test set:", flush=True)
with open("ST258_Mero_CoreAndAGEs_SVC_f1Tuned_Xtest_split{0}.pkl".format(str(args.split)), "rb") as f:
    Xtest = pickle.load(f)
print(Xtrain.head(), flush=True)
with open("ST258_Mero_CoreAndAGEs_SVC_f1Tuned_ytest_split{0}.pkl".format(str(args.split)), "rb") as f:
    ytest = pickle.load(f)
...
# Create pipeline to do both one-hot encoding and then SVC
pipe_CG_AG = Pipeline(steps=[
    ("transformer", transformer),
    ("svc", SVC(kernel='rbf', random_state=s, probability=True))
])
print("Pipeline:", flush=True)
print(pipe_CG_AG, flush=True)

# Create GSCV estimator
param_grid = {'svc__C': [10 ** x for x in range(-4,5)],
              'svc__gamma': [10 ** x for x in range(-8,4)],
...
with open('ST258_Mero_CoreAndAGEs_SVC_f1Tuned_split{0}_GridSearch.pkl'.format(str(args.split)), 'wb') as f:
...
Scores.to_csv("ST258_Mero_CoreAndAGEs_SVC_f1Tuned_split{0}_CVResults.csv".format(str(args.split)), index=False)
